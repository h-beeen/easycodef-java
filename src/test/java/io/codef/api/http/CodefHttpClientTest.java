...
public class CodefHttpClientTest {
	...
	void execute_Success() throws IOException {
		...
		when(mockConnection.getInputStream()).thenReturn(new ByteArrayInputStream(successBody.getBytes(StandardCharsets.UTF_8)));
	...
	void execute_Unauthorized() throws IOException {
		...
		when(mockConnection.getErrorStream()).thenReturn(new ByteArrayInputStream(errorBody.getBytes(StandardCharsets.UTF_8)));
