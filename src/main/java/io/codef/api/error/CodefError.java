...
public enum CodefError {
    ...
    PARSE_ERROR(
            "클라이언트가 서버 응답을 기대한 형식대로 파싱하지 못했습니다. 요청 형식을 확인해 주세요."
    ),

    RSA_ENCRYPTION_ERROR(
            "RSA 암호화 과정에서 오류가 발생했습니다. PublicKey 값을 확인해 주세요."
    ),
    ...
    INVALID_2WAY_KEYWORD(
            "추가 인증(2Way)을 위한 요청은 requestCertification메서드를 사용해야 합니다."
    ),
    ...
    IO_ERROR(
            "통신 요청에 실패했습니다. IP: 211.55.34.5, PORT: 443 방향의 Outbound 포트가 열려 있는지 확인해 주세요."
    ),
    SERVER_ERROR(
